...
export default function Chapter2({ searchParams }: SearchParamProps) {
  ...
  const handleGuess = () => {
    ...
    const distanceInMeters = getDistance({ latitude: guessLat, longitude: guessLng }, { latitude: data[0].lat, longitude: data[0].lng }, ACCURACY_IN_METERS);

    // compare guess lng and guess lat with data lng and lat
    if (distanceInMeters === 0) {
      // router.push('/chapter-2/win');
      console.log('good guess!');
    } else {
      // router.push('/chapter-2/lose');
  ...
  return (
    <>
      <div className="">
        <Map getLngLat={handleGetLngLat} lng={initialLng} lat={initialLat} />
